...
Remove $carbpressure$, $fillerlevel$, $ballinglvl$, $balling$, and $alchrel$ from train and test dataframe
...
# Define the list of columns excluding the 'ph' column
columns_to_include <- setdiff(names(train), "ph")
# Temporarily store the 'ph' column
ph_column <- train$ph
# Remove the 'ph' column
train$ph <- NULL
# Use the preProcess model to fill in missing values for the remaining data
preProc_without_ph <- preProcess(train[columns_to_include], method = "bagImpute")
# Use the preProcess model to fill in missing values
train <- predict(preProc_without_ph, newdata = train[columns_to_include])
# Restore the 'ph' column to the original data
train$ph <- ph_column
head(train)

columns_to_include <- setdiff(names(predict_me), "ph")
ph_column <- predict_me$ph
predict_me$ph <- NULL
preProc_without_ph <- preProcess(predict_me[columns_to_include], method = "bagImpute")
predict_me <- predict(preProc_without_ph, newdata = predict_me[columns_to_include])
predict_me$ph <- ph_column
head(predict_me)
```
...
ggplot(top15, aes(x = reorder(Variable, Scaled), y = Scaled)) +
  geom_bar(stat = "identity", fill = "#97c1a9", width = 0.7) +
  coord_flip() + 
  labs(x = "Variable", y = "Scaled Importance", title = "Top 15 Variable Importance") +
  theme_minimal() +
  theme(
...
Predict **ph** in **predict_me** dataset

```{r message=FALSE, warning=FALSE}
predict_me_vars <- predict_me[, names(training_data$x)]

ph_predictions <- predict(rfModel, newdata = predict_me_vars)

ph_predictions_rounded <- round(ph_predictions, 1)

predict_me$ph <- ph_predictions_rounded
